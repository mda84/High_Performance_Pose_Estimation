...
JOINT_LIMB = [[16, 14], [14, 0], [0, 15], [15, 17], [0, 1], [1, 2], [2, 3], [3, 4], [1, 5], [5, 6], [6, 7], [1, 8], [1, 11], [8, 9], [9, 10], [11, 12], [12, 13]]
...
def decode_pose(kpts, scale, image_original, state):
    ...
    #joint_list = [kpt*scale for kpt in kpts]
    joint_list = kpts
...
def decode_pose_old(humans, scale, image_original, state):
...
def body_pose_to_h36(body_pose_keypoints):
    ...
    for i in range (0, body_pose_keypoints.shape[0]):
        ...
        # Pelvis
        keypoints[0][0] = (body_pose_keypoints[i][8][0] + body_pose_keypoints[i][11][0]) / 2
        keypoints[0][1] = (body_pose_keypoints[i][8][1] + body_pose_keypoints[i][11][1]) / 2

        # Right Hip
        keypoints[1] = body_pose_keypoints[i][8]

        # Right Knee
        keypoints[2] = body_pose_keypoints[i][9]

        # Right Foot
        keypoints[3] = body_pose_keypoints[i][10]

        # Left Hip
        keypoints[4] = body_pose_keypoints[i][11]

        # Left Knee
        keypoints[5] = body_pose_keypoints[i][12]

        # Left Foot
        keypoints[6] = body_pose_keypoints[i][13]

        # Spine
        keypoints[7][0] = (body_pose_keypoints[i][2][0] + body_pose_keypoints[i][5][0] + body_pose_keypoints[i][8][0] + body_pose_keypoints[i][11][0]) / 4
        keypoints[7][1] = (body_pose_keypoints[i][2][1] + body_pose_keypoints[i][5][1] + body_pose_keypoints[i][8][1] + body_pose_keypoints[i][11][1]) / 4

        # Thorax
        keypoints[8] = body_pose_keypoints[i][1]

        # Nose
        keypoints[9] = body_pose_keypoints[i][0]

        # Head
        keypoints[10][0] = (body_pose_keypoints[i][14][0] + body_pose_keypoints[i][15][0]) / 2
        keypoints[10][1] = (body_pose_keypoints[i][14][1] + body_pose_keypoints[i][15][1]) / 2

        # Left Shoulder
        keypoints[11] = body_pose_keypoints[i][5]

        # Left Elbow
        keypoints[12] = body_pose_keypoints[i][6]

        # Left Wrist
        keypoints[13] = body_pose_keypoints[i][7]

        # Right Shoulder
        keypoints[14] = body_pose_keypoints[i][2]

         # Right Elbow
        keypoints[15] = body_pose_keypoints[i][3]

         # Right Wrist
        keypoints[16] = body_pose_keypoints[i][4]
