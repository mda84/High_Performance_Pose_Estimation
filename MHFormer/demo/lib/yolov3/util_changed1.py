...
def write_results(prediction, confidence, num_classes, nms=True, nms_conf=0.4, det_hm=False):
    ...
    for ind in range(batch_size):
        ...
        max_conf, max_conf_index = torch.max(image_pred[:, 4:4 + num_classes], 1)
        max_conf = max_conf.float().unsqueeze(1)
        #print(image_pred[:, :5])
        #print(max_conf)
        max_conf_index = max_conf_index.float().unsqueeze(1)
        #print(max_conf_index)
        seq = (image_pred[:, :4], max_conf, max_conf_index)
        #print(image_pred[:, :5].shape, max_conf.shape, max_conf_index.shape)
        ...
        #image_pred__ = image_pred[non_zero_ind.squeeze(), :].view(-1, 6)
        image_pred__ = image_pred.view(-1, 6)
        #print(image_pred__.shape)
        # filters out people id
        if det_hm:
            cls_mask = (image_pred__[:, -1] == 0).float()
            class_mask_ind = torch.nonzero(cls_mask).squeeze()
            image_pred_ = image_pred__[class_mask_ind].view(-1, 6)
        ...
        for cls in img_classes:
            ...
            image_pred_class = image_pred_[class_mask_ind].view(-1, 6)

            # sort the detections such that the entry with the maximum objectness
            # confidence is at the top
            conf_sort_index = torch.sort(image_pred_class[:, -2], descending=True)[1]
            ...
            if nms:
                ...
                for i in range(idx):
                    ...
                    non_zero_ind = torch.nonzero(image_pred_class[:, -2]).squeeze()
                    image_pred_class = image_pred_class[non_zero_ind].view(-1, 6)
                    #image_pred_class = image_pred_class.view(-1, 6)
